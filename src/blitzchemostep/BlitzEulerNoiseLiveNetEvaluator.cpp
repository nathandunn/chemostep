...
BlitzEulerNoiseLiveNetEvaluator::BlitzEulerNoiseLiveNetEvaluator(double newMinDeltaT,double newOutputSigmoid,NeuralNetwork* refNet,double newInputNoise,double newOutputNoise)  : BlitzEulerNetEvaluator()  // , FastSigLookup(245760,10000) 
{
    mSumH = 0 ; 
    ...
    mInputNoise = newInputNoise ; 
    mOutputNoise = newOutputNoise ; 
}

void BlitzEulerNoiseLiveNetEvaluator::setOutputSigmoid(double newSigmoid) 
{
    mOutputSigmoid = newSigmoid ; 
}

double BlitzEulerNoiseLiveNetEvaluator::getOutputSigmoid() 
{
...
void BlitzEulerNoiseLiveNetEvaluator::setNetwork(NeuralNetwork* newNetwork) 
{
//    cout << this << "  BlitzEulerNoiseLiveNetEvaluator setting the network to: " <<  *( ((BlitzNet*) newNetwork)->getWeights()) << endl ;  // this is run a lot
...
double BlitzEulerNoiseLiveNetEvaluator::getMinDeltaT() 
{
    return mMinDeltaT ; 
}


double BlitzEulerNoiseLiveNetEvaluator::evalPoint(vector<double>* stimVector,bool doTrace) 
{
    ...
    for(int i = 0 ; i < stimVector->size() ; i++){
        (*input)(i) += (*stimVector)[i] * (*evalGain)(i) + mInputNoise * RNG::normal() ;
    }
...
//    cout << "input value of output: " << (*input)(input->size()-1) << endl ; 
    ...
    (*evalNeurons) += ( ( (*sigma) - (*evalNeurons) ) / (*evalTau) ) * localDt + mOutputNoise * RNG::normal() ;
...
double BlitzEulerNoiseLiveNetEvaluator::fixOutput(double outputNeuron) 
{
...
//void BlitzEulerNoiseLiveNetEvaluator::relaxNet(bool doTrace) 
//{
//    cout << "BlitzEulerNoiseLiveNetEvaluator: trying to relax net: " << mRelaxTime << endl ; 
//    if(mRelaxTime==0) {
//        return ; 
//    }
//
...
void BlitzEulerNoiseLiveNetEvaluator::toXML(ostream *s) 
{
  // open the XML
  addSpacer(s,1) ; 
...
double BlitzEulerNoiseLiveNetEvaluator::getSumH() 
{
    return mSumH ; 
}
