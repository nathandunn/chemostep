...
//cout << "the header file" << headerFile << endl ; 
    ...
    LiveNet* aNet ;
    ErrorModel* tempErrorModel ; 
    


// instatiate every network in group
for(int i = 0 ; i < netList.size() ; i++){


    aNet = new LiveNet(util.loadNetwork(netFileList[i])) ; 
    tempErrorModel = aNet->getErrorModel() ;

    cout << "loading: " << netFileList[i] << endl ; 
    if(tempErrorModel->getType().find(DUAL_LIMIT)>=0){
        if(loFilter==0){
            tempLoFilter = (dynamic_cast<DualLimitErrorModel *>  (tempErrorModel))->getLowThresh() ; 
            cout << "setting low threshold from worm to: " << tempLoFilter << endl ; 
        }else{
            tempLoFilter = loFilter ; 
        }


        if(hiFilter==1){
            tempHiFilter = (dynamic_cast<DualLimitErrorModel *>  (tempErrorModel))->getHighThresh() ; 
            cout << "setting high threshold from worm to: " << tempHiFilter << endl ; 
        }else{
            tempHiFilter = hiFilter ; 
        }
    }
...
void printHelpAndExit() 
{
  ...
  cout << "  -randomize  	Randomizes the initial worm position.  Is off as a default." << endl;
