...
void Gnuplot::plot_xy(vector<double> x, vector<double> y, const string &title,const string &axes)
{
    ofstream tmp;
    ostringstream cmdstr;
    char name[] = "/tmp/gnuplotiXXXXXX";
    ...
    if (title == ""){
        cmdstr << "\"" << name << "\" axes " << axes << " with " << this->pstyle;
    }
    else{
        cmdstr << "\"" << name << "\" axes " << axes << " title \"" << title << "\" with " << this->pstyle ;
    }
